...
with open('GradientBoostingRegressor_model.pkl', 'rb') as file:
...
def adjust_predictions_based_on_priority(y_pred, total_amount, total_guests, priorities):
    for i in range(len(y_pred)):
        ...
        scaled_predictions = np.maximum(scaled_predictions, 0)
...
def predict_values_with_priorities(model, le_region, le_eventType, le_side, total_guests, region, eventType, side, amount, priorities):
    ...
    # Combine results and convert to integers
    results = {key: int(value) for key, value in zip(target_columns, predictions_adjusted[0])}
